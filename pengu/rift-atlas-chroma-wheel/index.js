...
(function createFakeChromaButton() {
  ...
  function hasKnownChromas(skinId) {
    ...
    // If this is a base skin (e.g. 233000), check if ANY skin for this champion has chromas
    if (numericId > 0 && numericId % 1000 === 0) {
      ...
      if (Number.isFinite(championId)) {
        ...
        if (championCache) {
          for (const [, entry] of championCache) {
            if (Array.isArray(entry.chromas) && entry.chromas.length > 0) {
              return true;
            }
          }
        }
  ...
  function getChromaData(skinData) {
    ...
    // Check bridge chroma cache (from CDTB skin index)
    const bridgeSkinId = getNumericId(baseSkinId ?? skinData.id);
    if (bridgeSkinId && bridgeChromaCache.has(bridgeSkinId)) {
      const bridgeChromas = bridgeChromaCache.get(bridgeSkinId);
      log.debug(
        `[getChromaData] Found ${bridgeChromas.length} bridge-cached chromas for skin ${bridgeSkinId}`
      );
      const championId = getChampionIdFromContext(skinData, bridgeSkinId, null)
        || skinData.championId
        || (Number.isFinite(bridgeSkinId) ? Math.floor(bridgeSkinId / 1000) : null);

      const defaultImagePath = championId && bridgeSkinId
        ? `/lol-game-data/assets/v1/champion-chroma-images/${championId}/${bridgeSkinId}.png`
        : null;
      ...
      const allChromas = [
        baseSkinChroma,
        ...bridgeChromas.map((c) => ({ ...c, selected: false })),
      ];
      return markSelectedChroma(allChromas, currentSkinId);
    }
  ...
  function selectChroma(
    ...
    if (window.fetch) {
      fetch("/lol-champ-select/v1/session/my-selection", {
        ...
        body: JSON.stringify({
          selectedSkinId: chroma.baseId || chroma.id,
        }),
      })
        .then(() => {
  ...
  function requestChromaFromBridge(skinData) {
    ...
    console.log("[LU-ChromaButton] requestChromaFromBridge sending", { championKey: String(championId), skinId: Number(skinId) });
    bridge.send({
  ...
  function handleChromaDataResponse(data) {
    console.log("[LU-ChromaButton] handleChromaDataResponse received", data);
    if (!data || !Array.isArray(data.chromas) || data.chromas.length === 0) { console.warn("[LU-ChromaButton] handleChromaDataResponse: invalid data"); return; }
    const championKey = String(data.championKey || "");
    const currentId = String(skinMonitorState?.championId || "");
    if (championKey && currentId && championKey !== currentId) { console.warn("[LU-ChromaButton] handleChromaDataResponse: champion mismatch", championKey, currentId); return; }

    const skinId = Number(data.skinId || 0);
    if (!skinId) return;

    const formatted = data.chromas.map((c) => {
      const colors = c.colors || [];
      return {
        id: Number(c.id || 0),
        baseId: Number(c.baseId || c.id || 0),
        name: c.name || "Chroma",
        imagePath: c.imagePath || "",
        colors,
        primaryColor: colors.length > 1 ? colors[1] : colors.length > 0 ? colors[0] : null,
        selected: false,
    ...
    const selectedId = pythonChromaState?.selectedChromaId != null
      ? pythonChromaState.selectedChromaId
      : skinMonitorState?.skinId || 0;
    const found = formatted.find((c) => c.id === Number(selectedId));
    if (found) found.selected = true;
    else formatted[0].selected = true;

    bridgeChromaCache.set(skinId, formatted);
    emitBridgeLog("chroma_data_cached", { skinId, count: formatted.length });

    if (document.getElementById(PANEL_ID) && lastPanelSkinData && lastPanelButtonElement) {
      createChromaPanel(lastPanelSkinData, formatted, lastPanelButtonElement);
    }
  }
  ...
  function handleSkinStateFromBridge(data) {
    ...
    if (prevState && prevState.skinId !== data.skinId) {
      selectedChromaData = null;
      updateChromaButtonColor();
    }
    if (data.championId && data.skinId) {
      ...
      if (!championSkinCache.has(championId)) {
        ...
        fetchChampionSkinData(championId).then(() => {
          log.info("[DIAG] champion data fetched, rescanning");
      ...
      requestChromaFromBridge({ championId: data.championId, skinId: data.skinId });
    }
    try { scanSkinSelection(); } catch (e) {}
  }
  ...
  function subscribeToSkinMonitor() {
    ...
    window.addEventListener("lu-skin-monitor-state", (event) => {
      ...
      if (detail && detail.championId && detail.skinId) {
        ...
        requestChromaFromBridge({ championId, skinId });
      }
  ...
  async function init() {
    ...
    try {
      ...
      if (bridge) {
        bridge.subscribe("skin-state", handleSkinStateFromBridge);
