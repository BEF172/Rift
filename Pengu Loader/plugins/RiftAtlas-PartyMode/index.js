/**
 * @name RiftAtlas-PartyMode
 * @author Rift Atlas
 * @description Rose-style Party Mode controls for Rift Atlas.
 * @link https://github.com/Alban1911/Rose
 */
(function initPartyMode() {
  const LOG_PREFIX = "[RiftAtlas-PartyMode]";

  const PANEL_ID = "rift-atlas-party-panel";
  const BUTTON_ID = "rift-atlas-party-button";
  const LOBBY_BUTTON_ID = "rift-atlas-party-lobby-button";
  ...
  async function loadBridgePort() {
    for (let attempt = 0; attempt < 50; attempt += 1) {
      if (window.__roseBridge) {
        sharedBridge = window.__roseBridge;
        return true;
      }
  ...
  async function handleToggleParty() {
    ...
    if (partyState.enabled) {
      ...
      sendBridgeMessage({ type: "party-disable", source: "rift-atlas-party-mode" });
    } else {
      // Enable
      toggleBtn.disabled = true;
      toggleBtn.innerHTML = '<span class="spinner"></span> Enabling...';
      sendBridgeMessage({ type: "party-enable", source: "rift-atlas-party-mode" });
    }
  }
  ...
  function handleAddPeer() {
    ...
    sendBridgeMessage({ type: "party-add-peer", source: "rift-atlas-party-mode", token: token });
    input.value = "";
  }

  // Global function for remove button onclick
  window.rosePartyRemovePeer = function (summonerId) {
    sendBridgeMessage({ type: "party-remove-peer", source: "rift-atlas-party-mode", summoner_id: summonerId });
  };
  ...
  function handleBridgeMessage(data) {
    ...
    switch (data.type) {
      ...
      case "party-peer-added": {
        ...
        sendBridgeMessage({ type: "party-get-state", source: "rift-atlas-party-mode" });
        break;
      }
  ...
  function connectBridge() {
    sharedBridge ||= window.__roseBridge;
    if (!sharedBridge) {
      setTimeout(connectBridge, 250);
      return;
    }
    if (!bridgeSubscribed) {
      bridgeSubscribed = true;
      ["party-state", "party-enabled", "party-disabled", "party-peer-added", "party-peer-removed", "phase-change"]
        .forEach((type) => sharedBridge.subscribe(type, handleBridgeMessage));
      sharedBridge.onReady(() => {
        bridgeReady = true;
        console.log(`${LOG_PREFIX} Shared Rose bridge connected on port ${sharedBridge.port}`);
        sendBridgeMessage({ type: "party-get-state", source: "rift-atlas-party-mode" });
      });
    }
    bridgeReady = Boolean(sharedBridge.ready);
  }
