...
const LOG_PREFIX = "[RiftAtlas-Jade]";
...
const initializeObserver = async () => {
    try {
        ...
        if (window.RiftAtlasJadeLog) {
            window.RiftAtlasJadeLog("error", "Failed to initialize observer", { error: error.message });
        }
    }
};
...
(() => {
    ...
    class RosePlugin {
        ...
        async init() {
            ...
            if (window.RiftAtlasJadeLog) {
                window.RiftAtlasJadeLog("info", "RiftAtlas-Jade plugin initializing");
            }
